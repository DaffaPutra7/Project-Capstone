<x-guest-layout>
    ...
    <div class="grid md:grid-cols-2 gap-10 max-w-5xl w-full mx-auto items-center h-screen md:h-auto overflow-hidden md:overflow-visible">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4" novalidate>
                ...
                <div x-data="{ hasTyped: false }">
                    ...
                    <textarea id="alamat" 
                              ...
                              rows="3"
                              @input="hasTyped = true"
                              :class="(!hasTyped && {{ $errors->has('alamat') ? 'true' : 'false' }}) ? 'border-red-500' : 'border-gray-300'"
                              >{{ old('alamat') }}</textarea>
                    <div x-show="!hasTyped">
                        @error('alamat')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
